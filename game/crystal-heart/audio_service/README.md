...
### Getting High-Resolution Normalized Bands

```gdscript
# /audio/bands_normalized sends one JSON payload with active high-res bands
var bands_payload = osc_receiver.get_normalized_bands("/audio/bands_normalized")
if not bands_payload.is_empty():
    var total = bands_payload.get("total", 0)
    var active = bands_payload.get("active", 0)
    var bands = bands_payload.get("bands", [])
    # each band item: { "index": int, "normalized": float, "ema": float }
```
...
- `/audio/bands_normalized` - JSON string with high-resolution active normalized bands (use `get_normalized_bands()`)

## Clock + Bands-Only Mode

When running:

```bash
./run_audio_service.sh --clock-and-bands-only --normalized-band-count 128
```
...
In this mode, many individual `/audio/*` breakout endpoints are intentionally not sent.
...
4. **Type Safety**: Use the typed getters (`get_float()`, `get_int()`, `get_bool()`) for safer access with default values.
   For `/audio/bands_normalized`, use `get_normalized_bands()` so compact band tuples are normalized into dictionaries.
